...
SELECT R.player_id, COUNT(DISTINCT R.year), MAX(W.position) AS pos
INTO bw_diag
...
INTO bw_diff
FROM (
SELECT A.player_id, A.count AS total_years, COALESCE(D.count,0) AS war_years, COALESCE(D.pos,'Unknown') AS pos
FROM (
